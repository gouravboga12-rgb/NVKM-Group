...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 page-transition">
      ...
      {/* ── HERO ── */}
      <section className="relative overflow-hidden py-10 sm:py-20 lg:py-32 bg-gradient-to-br from-[#0c2f18] via-[#103d21] to-[#04160a] rounded-[24px] sm:rounded-[40px] text-white shadow-2xl mt-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10 px-4 xs:px-6 sm:px-12 lg:px-16">
          <div className="lg:col-span-7 space-y-6 lg:space-y-7 text-center lg:text-left">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-2">
              <button onClick={() => navigate('/shop')} className="bg-[#22C55E] hover:bg-[#16A34A] text-white font-extrabold px-6 sm:px-8 py-3.5 sm:py-4 rounded-2xl shadow-xl shadow-green-950/40 transition-all duration-300 hover:scale-[1.02] flex items-center gap-2 text-xs uppercase tracking-wider">
            ...
            <div className="relative max-w-sm mx-auto bg-white/5 backdrop-blur-xl border border-white/10 p-5 rounded-[24px] sm:rounded-[40px] shadow-2xl animate-float">
              ...
              <div className="mt-5 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-xs text-[#4ADE80] font-bold">⭐ 4.9 · Bestseller Powder</span>
                </div>
                <div className="bg-[#4ADE80]/15 border border-[#4ADE80]/30 px-3.5 py-1.5 rounded-2xl text-[#4ADE80] text-sm font-extrabold shadow-sm">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5">
        {[
          ...
          <div key={label} className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-4 sm:p-7 text-center shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-[1.02] border-glow-green">
            <div className="font-heading font-extrabold text-2xl sm:text-4xl text-[#14532D] tracking-tight">{num}</div>
      ...
      <section className="py-4">
        <div className="text-center max-w-2xl mx-auto space-y-2 mb-12">
          <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Healthy &amp; Nutritious</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={title} className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300">
              <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-[#14532D] flex items-center justify-center text-xl mb-6 shadow-inner border border-emerald-100/50">
      ...
      <section className="py-4">
        <div className="text-center max-w-2xl mx-auto space-y-2 mb-12">
          <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Explore Range</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-5">
          {categories.map((cat, index) => (
            <div
              ...
              className="group bg-white border border-slate-100 p-4 sm:p-6 rounded-[20px] sm:rounded-[30px] hover:border-[#4ADE80] hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col justify-between shadow-sm"
            >
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-tr ${cat.color} flex items-center justify-center text-[#14532D] text-xl group-hover:scale-110 transition-transform duration-300 shadow-sm border border-black/5`}>
                <i className={cat.icon} />
              </div>
              <div className="mt-5">
                <h3 className="font-heading font-extrabold text-sm md:text-base text-[#111827] group-hover:text-[#14532D] transition-colors">{cat.name}</h3>
                <p className="text-[10px] text-slate-400 mt-2.5 leading-relaxed font-semibold">{cat.text}</p>
              </div>
              <span className="text-xs font-bold text-[#14532D] flex items-center gap-1.5 mt-5 group-hover:translate-x-1.5 transition-all">
      ...
      <section className="py-4">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10">
          <div>
            <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Best Sellers</span>
            <h2 className="font-heading font-extrabold text-3xl text-[#111827] mt-1">Featured Products</h2>
          </div>
          <button onClick={() => navigate('/shop')} className="text-sm font-bold text-[#14532D] hover:text-[#22C55E] flex items-center gap-1.5 mt-4 sm:mt-0 transition-colors">
      ...
      {/* ── WHOLESALE BANNER ── */}
      <section className="py-4">
        <div className="bg-gradient-to-tr from-[#14532D] to-emerald-900 text-white rounded-[24px] sm:rounded-[35px] p-6 sm:p-8 md:p-12 shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
          <div className="absolute right-0 bottom-0 top-0 w-1/3 bg-[radial-gradient(circle_at_right_bottom,rgba(74,222,128,0.15),transparent)] pointer-events-none" />
          <div className="flex-1 space-y-4 text-center md:text-left">
            <span className="bg-emerald-800 text-[#4ADE80] border border-emerald-700/50 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Wholesale &amp; Retail Supplier</span>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3 pt-2">
              <a href={`https://wa.me/9014274293?text=${encodeURIComponent('Hello NVKM GROUP, I am interested in bulk wholesale rates. Please share your catalog.')}`} target="_blank" rel="noreferrer" className="bg-[#22C55E] hover:bg-[#16A34A] text-white font-bold text-xs py-3 px-6 rounded-xl transition-all shadow-md flex items-center gap-2">
          ...
          <div className="w-full md:w-64 flex flex-col gap-3 p-5 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md shrink-0 text-left">
            <h3 className="font-heading font-bold text-sm border-b border-white/10 pb-2 flex items-center gap-1.5">
              <i className="fa-solid fa-truck-fast text-[#4ADE80]" /> Bulk Order Specs
            </h3>
            <ul className="space-y-2 text-xs font-light text-slate-200">
              {['Min Order: 5 kg', 'Custom bag / label size', 'Up to 40% bulk discount', 'Lab purity test reports', 'PAN India shipping'].map(item => (
                <li key={item} className="flex items-center gap-2">
                  <i className="fa-solid fa-circle text-[6px] text-[#4ADE80]" /> {item}
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="py-4">
        <div className="text-center max-w-2xl mx-auto space-y-2 mb-10">
          <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Our Commitment</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            ...
            <div key={title} className="bg-white p-6 rounded-3xl border border-slate-100 text-center flex flex-col items-center shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 text-[#14532D] flex items-center justify-center text-lg mb-4">
                <i className={`fa-solid ${icon}`} />
              </div>
      ...
      <section className="py-4">
        <div className="text-center max-w-2xl mx-auto space-y-2 mb-10">
          <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Verified Reviews</span>
          ...
          {[
            ...
            <div key={name} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4 border-t pt-3 border-slate-50 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center font-bold text-[#14532D] text-xs">{initial}</div>
                <div>
      ...
      <section className="py-4 mb-6">
        <div className="text-center max-w-2xl mx-auto space-y-2 mb-10">
          <span className="text-xs font-bold text-[#22C55E] tracking-widest uppercase">Healthy Inspirations</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="aspect-square bg-slate-100 rounded-3xl overflow-hidden relative group">
              ...
              <div className="absolute inset-0 bg-[#14532D]/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2">

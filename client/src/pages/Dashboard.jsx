...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8 page-transition">
      ...
      <div
        data-aos="fade-down"
        className="flex flex-wrap justify-between items-center gap-4 bg-gradient-to-r from-emerald-950 to-emerald-800 rounded-[20px] xs:rounded-[28px] p-5 xs:p-8 md:p-10 text-white relative overflow-hidden shadow-xl shadow-emerald-950/20"
      >
        <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full bg-radial-gradient(circle,rgba(74,222,128,.1),transparent) pointer-events-none" />
        <div className="relative z-10">
          <p className="text-[11px] font-extrabold text-emerald-200/70 uppercase tracking-widest mb-1">My Account</p>
          <h1 className="font-heading font-black text-xl xs:text-2xl md:text-3xl lg:text-4xl tracking-tight leading-none">
            Welcome back, {user.name?.split(' ')[0]}! 👋
          </h1>
          <p className="text-[0.82rem] text-emerald-100/80 mt-1.5 font-medium">Access your profile, orders, and delivery statuses.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4" data-aos="fade-up">
        {[
          ...
          { icon: 'fa-circle-check', label: 'Delivered', value: deliveredOrders, textClass: 'text-emerald-600', bgClass: 'bg-emerald-50 border-emerald-100' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div
          data-aos="fade-right"
          className="lg:col-span-4 glass-premium rounded-[20px] xs:rounded-[28px] p-4 xs:p-6 sm:p-8 shadow-xl border border-emerald-500/10"
        >
          {/* Avatar */}
          <div className="text-center mb-6 pb-6 border-b border-slate-100">
            <div className="w-16 h-16 rounded-full mx-auto mb-3 bg-gradient-to-br from-emerald-900 to-emerald-500 flex items-center justify-center font-heading font-black text-2xl text-white shadow-lg shadow-emerald-950/20 ring-4 ring-emerald-500/10">
              {avatarLetter}
            </div>
            <h3 className="font-heading font-extrabold text-base text-slate-900">{user.name}</h3>
            <span className="inline-flex items-center bg-emerald-50 border border-emerald-100 text-[#15803d] text-[9px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider mt-2 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            {[
              ...
              <div key={label} className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50/50 transition-colors">
                <div className="w-9 h-9 rounded-lg bg-emerald-50/60 border border-emerald-100/50 flex items-center justify-center flex-shrink-0">
                  <i className={`fa-solid ${icon} text-emerald-600 text-sm`} />
                </div>
        ...
        <div
          data-aos="fade-left"
          className="lg:col-span-8 glass-premium rounded-[20px] xs:rounded-[28px] p-4 xs:p-6 sm:p-8 shadow-xl border border-emerald-500/10"
        >
          <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-900 to-emerald-500 flex items-center justify-center shadow-md">
            ...
            {fetching ? (
              ...
              orders.map((ord) => {
                ...
                return (
                  <div
                    key={ord.orderId}
                    className="bg-white border border-slate-100 rounded-xl xs:rounded-2xl p-4 xs:p-5 shadow-sm hover:shadow-md hover:border-emerald-500/30 hover:bg-slate-50/10 transition-all duration-200"
                  >
                    {/* Order Header */}
                    <div className="flex flex-wrap justify-between items-center gap-3 mb-4 pb-4 border-b border-slate-100">
                      <div>
                        ...
                        <span className="text-[0.9rem] font-black text-emerald-900 ml-2 font-heading tracking-wider">{ord.orderId}</span>
                      </div>
                    ...
                    <div className="flex flex-wrap justify-between gap-4 mb-5">
                        ...
                        <div className="flex flex-col gap-1.5">
                          {ord.items.map((itm, idx) => (
                            <span key={idx} className="text-[0.82rem] text-slate-700 font-semibold flex items-center gap-2">
                              <i className="fa-solid fa-circle-dot text-[6px] text-emerald-500" />
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <span className="font-heading font-black text-lg text-emerald-900 font-heading">₹{ord.totalPayable.toFixed(2)}</span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-4">
                      <span className="block text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-3">
                        <i className="fa-solid fa-location-dot mr-1.5 text-emerald-500 animate-pulse" /> Live Delivery Tracker
                      </span>
              ...
              <div className="text-center py-12 px-6 border-2 border-dashed border-emerald-500/20 rounded-2xl bg-emerald-50/20">
                <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-4">
                  <i className="fa-solid fa-box-open text-emerald-600 text-2xl" />
                </div>
                ...
                <button
                  ...
                  className="mt-5 bg-gradient-to-r from-emerald-900 to-emerald-600 text-white font-extrabold py-3 px-6 rounded-2xl text-[0.82rem] cursor-pointer shadow-lg shadow-emerald-950/20 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-emerald-950/30 active:translate-y-0 transition-all duration-200"
                >
